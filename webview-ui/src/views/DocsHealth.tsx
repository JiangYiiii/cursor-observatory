...
import type { DocsHealth as DocsHealthModel } from "@/types/observatory";
import { DocsWorkspaceSection } from "@/views/DocsWorkspaceSection";
...
export function DocsHealth() {
  ...
  return (
    <div className="min-h-0 flex-1 space-y-6 overflow-y-auto">
      <DocsWorkspaceSection />
      {isLoading && !docsHealth ? (
        ...
        <HealthScoreCard docsHealth={docsHealth} />
      )}
    </div>
  );
}

function HealthScoreCard({ docsHealth }: { docsHealth: DocsHealthModel }) {
    ...
    >
      <div className="mb-6 flex flex-wrap items-end gap-4">
        ...
        <p className="max-w-xl text-sm text-zinc-600 dark:text-zinc-300">
          得分来自多项检查加权；点击下表行可展开{" "}
          <code className="text-xs">details</code> 查看缺失模块、失效路径等明细。
        </p>
      </div>
